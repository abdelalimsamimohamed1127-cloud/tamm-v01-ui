...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const AgentCredits = () => {
    ...
    const used = currentAgent?.creditsUsed ?? 0;
    const limit = currentAgent?.creditsLimit ?? 50;
    const resetDate = currentAgent?.creditsReset ?? 'Renews monthly';
      ...
      <div className="rounded-lg border border-sidebar-border bg-background shadow-sm p-3">
        ...
        {!collapsed && (
          <div className="space-y-2 mt-2">
            ...
            <p className="text-xs text-muted-foreground">Reset date: {resetDate}</p>
